...
class UserCrudForm extends Component
{
  ...
  public function update()
  {
    ...
    DB::transaction(function () use ($validated, $id) {
      ...
      $userPayload = [
        'name'         => $validated['name'],
        'email'        => $validated['email'],
        'is_activated' => (int) ($validated['is_activated'] ?? $user->is_activated),
        'queue_number' => $validated['queue_number'] ?? $user->queue_number,
      ];
  ...
  {
    // IMPORTANT: chart objects dibuat di sini, bukan public property
    $charts = $this->buildCharts();

    return view('livewire.user.components.user-crud-form', $charts)
      ->title($this->title);
  }
}
